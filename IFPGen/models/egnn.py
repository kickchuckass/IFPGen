...
from torch_geometric.nn import radius_graph, knn_graph, global_mean_pool, global_max_pool, global_add_pool
...
class EnBaseLayer(nn.Module):
    ...
    def forward(self, h, x, edge_index, edge_attr=None):
        ...
        if self.update_x:
            ...
            x = x + delta_x   # only ligand positions will be updated

        return h, x


class EGNN(nn.Module):
    def __init__(self, num_layers, hidden_dim, edge_feat_dim, num_r_gaussian, k=10, cutoff=10.0, cutoff_mode='knn',
                 update_x=False, act_fn='silu', norm=False, pool_type = 'mean'):
        super().__init__()
        # Build the network
