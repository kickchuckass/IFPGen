...
# Model
class IFPDiff(nn.Module):

    def __init__(self, config, protein_atom_feature_dim, ligand_atom_feature_dim, IFP_class = 8):
        super().__init__()
        self.config = config
        ...
        # ---------------------------------------------------------------------
        self.if_ifp = self.config.if_ifp
        self.if_catifp_basic = self.config.if_catifp_basic
        self.hidden_ifp = self.config.hidden_ifp

        if self.if_ifp:
            if self.if_catifp_basic:
                self.protein_combine_emb = nn.Linear(protein_atom_feature_dim+IFP_class, emb_dim)
            else:
                self.protein_atom_emb = nn.Linear(protein_atom_feature_dim, emb_dim)
                self.protein_emb_combine = nn.Linear(emb_dim*2, emb_dim)

            self.protein_ifp_emb = nn.Linear(IFP_class, self.hidden_dim)
            
        else:
            self.protein_atom_emb = nn.Linear(protein_atom_feature_dim, emb_dim)

        #---------------------------------------------------------------------
        self.if_predict_loss = self.config.if_predict_loss
        #---------------------------------------------------------------------
        if self.if_predict_loss:
            self.protein_atom_predict = nn.Sequential(
                ...
                nn.GELU(),
                nn.Dropout(0.1),
                nn.Linear(self.hidden_dim * 2, 2, bias=False)
            )
            self.p_atom_loss = nn.CrossEntropyLoss()
            self.loss_predict_weight = getattr(config, 'loss_predict_weight', 0.5)
        # ---------------------------------------------------------------------
        ...
        if config.refer_ligand_cat:
            self.refer_ligand_input_embed = nn.Linear(ligand_atom_feature_dim, emb_dim)

        # TODO
        if config.virtual_refer_node:
            self.unimol_feat_trans = nn.Linear(512, emb_dim)

    def forward(self, protein_pos, protein_v, batch_protein, init_ligand_pos, init_ligand_v, batch_ligand,
            refer_ligand_pos, refer_ligand_v, refer_ligand_batch, unimol_repr,
            time_step=None, return_all=False, fix_x=False, ifp_info=None, k=10,
            gen_flag = None,
            ):
        ...
        refer_ligand_h = None
        refer_ligand_h_extended = None
        if self.config.virtual_refer_node:
            if self.config.refer_ligand_cat:
                refer_ligand_v = F.one_hot(refer_ligand_v, self.num_classes).float()
                if self.time_emb_dim > 0:
                    if self.time_emb_mode == 'simple':
                        input_refer_ligand_feat = torch.cat([refer_ligand_v, (time_step / self.num_timesteps)[refer_ligand_batch].unsqueeze(-1)], -1)
                    elif self.time_emb_mode == 'sin':
                        time_feat = self.time_emb(time_step)
                        input_refer_ligand_feat = torch.cat([refer_ligand_v, time_feat[refer_ligand_batch]], -1)
                    else:
                        raise NotImplementedError
                else:
                    input_refer_ligand_feat = refer_ligand_v
                
                refer_ligand_h = self.refer_ligand_input_embed(input_refer_ligand_feat)

                if not self.config.hidden_refer_ligand:
                    refer_ligand_h_extended = refer_ligand_h[batch_ligand]
                    assert refer_ligand_h_extended.size(0) == init_ligand_h.size(0), "Batch mapping mismatch!"
                    
                    if self.config.out_add_refer_ligand:
                        init_ligand_h += refer_ligand_h_extended
        ...
        if not self.if_ifp:
            ...
            if self.if_catifp_basic:
                h_protein = self.protein_combine_emb(torch.cat([protein_v, ifp_info], dim=1))
            else:
                h_protein_basic = self.protein_atom_emb(protein_v)
                interaction = self.protein_ifp_emb(ifp_info)
                h_protein = self.protein_emb_combine(torch.cat([h_protein_basic, interaction], dim=1))
            
            if self.hidden_ifp:
                raw_interaction = torch.zeros((len(protein_v), self.hidden_dim)).to(protein_v.device)
                idx_with_ifp = (ifp_info != 0).any(dim=1)
                filter_ifp_info = ifp_info[idx_with_ifp].to(protein_v.device).float()
                filter_inter = self.protein_ifp_emb(filter_ifp_info)
                raw_interaction[idx_with_ifp] = filter_inter
        ...
        if self.config.node_indicator:
            ...
            if self.config.virtual_refer_node:
                refer_ligand_h = torch.cat([refer_ligand_h, torch.zeros(len(refer_ligand_h), 1).to(h_protein)], -1)
                if self.config.hidden_refer_ligand:
                    refer_ligand_h_extended = torch.cat([refer_ligand_h_extended, torch.ones(len(init_ligand_h), 1).to(h_protein)], -1)
        ...
        if gen_flag is not None:
            update_mask = torch.zeros_like(mask_ligand, dtype=torch.bool)
            update_mask[mask_ligand] = gen_flag
        else:
            update_mask = mask_ligand.clone() 

        input_refer_ligand_h = refer_ligand_h

        outputs = self.refine_net(h_all, pos_all, mask_ligand, batch_all, return_all=return_all, 
                                fix_x=fix_x, interaction=raw_interaction, hidden_refer_ligand=input_refer_ligand_h,
                                refer_ligand_pos=refer_ligand_pos, use_virtual_refer_node=self.config.virtual_refer_node, 
                                refer_ligand_batch=batch_ligand,
                                gen_flag = update_mask,
    ...
    def get_diffusion_loss(
            self, protein_pos, protein_v, batch_protein, ligand_pos, ligand_v, batch_ligand, time_step=None, ifp_info = None, unimol_repr = None,
    ):
        ...
        preds = self.forward(
            ...
            refer_ligand_pos=ligand_pos,
            refer_ligand_v=ligand_v,
            refer_ligand_batch=batch_ligand,
            unimol_repr = unimol_repr,
        )

        if self.if_predict_loss:
            idx_with_ifp = (ifp_info != 0).any(dim=1)
            protein_h = preds['protein_h']
            inter_preds = self.protein_atom_predict(protein_h)
            p_atom_loss = self.p_atom_loss(inter_preds,idx_with_ifp.long())
        # -------------------------------------------------------------
        ...
        if self.if_predict_loss:
            loss = loss_pos + loss_v * self.loss_v_weight + p_atom_loss * self.loss_predict_weight
        else:
    ...
    def sample_diffusion(self, protein_pos, protein_v, batch_protein,
                         ...
                         num_steps=None, center_pos_mode=None, pos_only=False, ifp_info= None,
                         energy_drift_opt=None,full_protein_pos=None,full_batch_protein=None,
                         refer_ligand_pos = None, refer_ligand_v = None, refer_ligand_batch = None, unimol_repr = None,
                         ):
        ...
        for i in tqdm(time_seq, desc='sampling', total=len(time_seq),miniters= 100, mininterval = 100):
            ...
            preds = self.forward(
                ...
                refer_ligand_pos=refer_ligand_pos,
                refer_ligand_v=refer_ligand_v,
                refer_ligand_batch=refer_ligand_batch,
