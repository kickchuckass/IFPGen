...
class AttentionLayerO2TwoUpdateNodeGeneral(nn.Module):
    ...
    def forward(self, h, x, edge_attr, edge_index, mask_ligand, e_w=None, fix_x=False, interaction=None, hidden_refer_ligand=None,
                #829
                gen_flag = None,
                ):
        src, dst = edge_index
...
class UniTransformerO2TwoUpdateGeneral(nn.Module):
    def __init__(self, num_blocks, num_layers, hidden_dim, n_heads=1, k=32,
                 ...
                 cutoff_mode='radius', ew_net_type='r', use_virtual_refer_node = False,
        ...
        self.use_virtual_refer_node = use_virtual_refer_node
        if self.use_virtual_refer_node:
    ...
    def _connect_edge(self, x, mask_ligand, batch, use_virtual_refer_node = False,
                      hidden_refer_ligand = None, refer_ligand_pos = None, refer_ligand_batch = None):
        ...
        if use_virtual_refer_node and hidden_refer_ligand is not None and refer_ligand_pos is not None:
            new_edges = self.add_virtual_edges(x, mask_ligand, batch, hidden_refer_ligand, refer_ligand_batch = refer_ligand_batch)
            if new_edges is not None:
                edge_index = torch.cat([edge_index, new_edges], dim=1)
        return edge_index
    
    def add_virtual_edges(self, x, mask_ligand, batch, hidden_refer_ligand, refer_ligand_batch=None):
        N = x.shape[0]
        batch_size = len(torch.unique(batch))
        
        if hidden_refer_ligand.dim() == 1:
            hidden_refer_ligand = hidden_refer_ligand.unsqueeze(0).expand(batch_size, -1)
        
        batch_offset = torch.cumsum(torch.bincount(batch), dim=0)[:-1]  
        new_edges = []

        for b in range(batch_size):
            mask_b = batch == b
            N_b = mask_b.sum().item()  
            virtual_indices = refer_ligand_batch[batch_offset[b]:batch_offset[b] + N_b]  
            
            for i in range(N_b):
                if mask_ligand[batch_offset[b] + i]:
                    for virtual_idx in virtual_indices:
                        new_edges.append([virtual_idx, batch_offset[b] + i])  
                        new_edges.append([batch_offset[b] + i, virtual_idx])  
        
        return torch.tensor(new_edges, dtype=torch.long).t() if new_edges else None

    @staticmethod
    def _build_edge_type(edge_index, mask_ligand, use_virtual_node = False, refer_ligand_batch = None):
        src, dst = edge_index
        ...
        if use_virtual_node:
            edge_type = UniTransformerO2TwoUpdateGeneral.extend_edge_type_with_virtual(edge_index, mask_ligand, edge_type)
        return F.one_hot(edge_type, num_classes=5 if use_virtual_node else 4)

    @staticmethod
    def extend_edge_type_with_virtual(edge_index, mask_ligand, edge_type):
        src, dst = edge_index
        virtual_mask = (src >= mask_ligand.shape[0]) | (dst >= mask_ligand.shape[0])
        edge_type[virtual_mask] = 4  # virtual-ligand or ligand-virtual
        return edge_type
    # ---------------------------
    
    def forward(self, h, x, mask_ligand, batch, return_all=False, fix_x=False, 
                interaction = None, hidden_refer_ligand = None, 
                refer_ligand_pos = None, use_virtual_refer_node = False,refer_ligand_batch = None,
                gen_flag = None,
                ):
        all_x = [x]
        all_h = [h]
        for b_idx in range(self.num_blocks):
            ...
            edge_index = self._connect_edge(x, mask_ligand, batch, use_virtual_refer_node, hidden_refer_ligand, refer_ligand_batch = refer_ligand_batch)
            src, dst = edge_index
            if use_virtual_refer_node and hidden_refer_ligand is not None and refer_ligand_pos is not None:
                h, x, extended_mask = self.add_virtual_nodes(h, x, mask_ligand, batch, hidden_refer_ligand, refer_ligand_pos, refer_ligand_batch)
                if gen_flag is not None:
                    update_mask = torch.zeros_like(extended_mask, dtype=torch.bool)
                    update_mask[:mask_ligand.shape[0]] = gen_flag
                    gen_flag = update_mask
            else:
                extended_mask = mask_ligand
            edge_type = self._build_edge_type(edge_index, extended_mask, use_virtual_refer_node, refer_ligand_batch)
            if use_virtual_refer_node and interaction != None:
                supply_interaction = torch.zeros(hidden_refer_ligand.shape[0], self.hidden_dim).to(h.device)
                interaction = torch.cat([interaction, supply_interaction], dim=0).to(h.device)
            ...
            for l_idx, layer in enumerate(self.base_block):
                h, x = layer(h, x, edge_type, edge_index, extended_mask, e_w=e_w, 
                        fix_x=fix_x, interaction = interaction, hidden_refer_ligand = hidden_refer_ligand
                        ,gen_flag = gen_flag,
                        )
            all_x.append(x)
            all_h.append(h)

        N = mask_ligand.shape[0]
        outputs = {'x': x[:N], 'h': h[:N]}
        if return_all:
            outputs.update({'all_x': [x[:N] for x in all_x], 'all_h': [h[:N] for h in all_h]})
        return outputs

    def add_virtual_nodes(self, h, x, mask_ligand, batch, hidden_refer_ligand, pocket_center, refer_ligand_batch=None):
        N = h.shape[0]
        if hidden_refer_ligand.dim() == 1:
            batch_size = len(torch.unique(batch))
            hidden_refer_ligand = hidden_refer_ligand.unsqueeze(0).expand(batch_size, -1)  # [B, dim]
            pocket_center = pocket_center.unsqueeze(0).expand(batch_size, -1)             # [B, 3]

        virtual_h = hidden_refer_ligand  # [N_v, dim]
        virtual_x = pocket_center

        # 拼接节点特征和坐标
        h = torch.cat([h, virtual_h], dim=0)  # [N + N_v, dim]
        x = torch.cat([x, virtual_x], dim=0)  # [N + N_v, 3]
        num_virtual = virtual_h.shape[0]
        extended_mask = torch.cat([
            mask_ligand,
            torch.zeros(num_virtual, dtype=torch.bool, device=mask_ligand.device)
        ], dim=0)

        return h, x, extended_mask