...
def plot_distance_hist(pair_length_profile, metrics=None, save_path=None, colors=None):
    gt_profile = eval_bond_length_config.PAIR_EMPIRICAL_DISTRIBUTIONS
    plt.figure(figsize=(6 * len(gt_profile), 4))

    for idx, (k, gt_distribution) in enumerate(gt_profile.items()):
        plt.subplot(1, len(gt_profile), idx + 1)
        x = eval_bond_length_config.PAIR_EMPIRICAL_BINS[k]

        if colors is not None:
            true_color, learned_color = colors
        else:
            true_color, learned_color = 'blue', 'red' 

        plt.step(x, gt_distribution[1:], color=true_color)
        plt.step(x, pair_length_profile[k][1:], color=learned_color)
        # plt.legend(['True', 'Learned'])
