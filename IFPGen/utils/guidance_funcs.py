...
from .chem import ff_optimize, get_ring_systems
...
def compute_atom_distance_loss(pos, e_index,e_type, min_d=1.2, max_d=1.9):
    if torch.any(e_type>0):
        bond_index=e_index[:,e_type>0]
        bond_len= torch.norm(pos[bond_index[0]] - pos[bond_index[1]], dim=-1)
        # min_dist_all, _ = scatter_min(bond_len, e_index, dim=0)
        # min_dist, min_dist_sca_idx = min_dist_all.min(-1)
        # loss_armsca_prox += torch.mean(pairwise_dist)
        # 1.2 < min dist < 1.9
        loss = torch.mean(torch.clamp(min_d - bond_len, min=0) + torch.clamp(bond_len - max_d, min=0))
    else:
        loss = torch.tensor(0., device=pos.device)
    return loss

def compute_batch_distance_loss(pred_ligand_pos, batch_ligand, ligand_edge_index, ligand_edge_type, batch_ligand_bond,
                                distance_min=1.2, distance_max=1.9, epsilon1=0.1, epsilon2=0.1, sigma=0.1):
    # Initialize total distance loss
    loss_distance = torch.tensor(0., device=pred_ligand_pos.device)
    ...
    for i in range(num_graphs):
        ...
        # Select ligand atom positions for the current graph
        l_pos = pred_ligand_pos [batch_ligand == i]
        e_index = ligand_edge_index[:, batch_ligand_bond == i]-(batch_ligand == i).nonzero()[0].min()
        e_type = ligand_edge_type[batch_ligand_bond == i].argmax(dim=-1)
        if e_index.shape[1] == 0:
            continue
        loss_distance+= compute_atom_distance_loss(l_pos, e_index, e_type,min_d=distance_min, max_d=distance_max)
    return loss_distance / num_graphs


def compute_angle_loss(pos, e_index, e_type, min_d=1.2, max_d=1.9, angle_min=90):
    min_d= min_d * 2 * math.sin(PI * angle_min / 360)
    max_d= max_d * 2 
    if torch.any(e_type>0):
        bond_index=e_index[:,e_type>0].squeeze()
        # bond_len= torch.norm(pos[bond_index[0]] - pos[bond_index[1]], dim=-1)
        e_full_idx = torch.cat([bond_index, bond_index[[1, 0]]], dim=1)
        adj = to_dense_adj(e_full_idx, max_num_nodes=pos.shape[0])[0, :, :]
        adj2 = adj @ adj
        trace_zero = (1 - torch.eye(pos.shape[0])).to(adj2.device)
        adj2 = adj2 * trace_zero  # remove self-loop
        e_two_hop_index = adj2.nonzero().contiguous().T
        
        d = torch.norm(pos[e_two_hop_index[0]] - pos[e_two_hop_index[1]], dim=1)
        # min_dist_all, _ = scatter_min(bond_len, e_index, dim=0)
        # min_dist, min_dist_sca_idx = min_dist_all.min(-1)
        # loss_armsca_prox += torch.mean(pairwise_dist)
        # 1.2 < min dist < 1.9
        loss = torch.mean(torch.clamp(min_d - d, min=0) + torch.clamp(d - max_d, min=0))
    else:
        loss = torch.tensor(0., device=pos.device)
    return loss

def compute_batch_angle_loss(pred_ligand_pos, batch_ligand, ligand_edge_index, ligand_edge_type, batch_ligand_bond,
                                distance_min=1.2, distance_max=1.9, epsilon1=0.1, epsilon2=0.1, sigma=0.1):
    # Initialize total distance loss
    loss_distance = torch.tensor(0., device=pred_ligand_pos.device)
    ...
    for i in range(num_graphs):
        ...
        if e_index.shape[1] == 0:
            continue
        loss_distance+= compute_angle_loss(l_pos, e_index, e_type,min_d=distance_min, max_d=distance_max)
    return loss_distance / num_graphs
