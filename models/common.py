...
def compose_context_with_virtual_refer_node(h_protein, h_ligand, pos_protein, pos_ligand, 
                                            batch_protein, batch_ligand, refer_ligand_pos, refer_ligand_h, refer_ligand_batch):
    # previous version has problems when ligand atom types are fixed
    # (due to sorting randomly in case of same element)

    batch_ctx = torch.cat([batch_protein, batch_ligand, refer_ligand_batch], dim=0)
    # sort_idx = batch_ctx.argsort()
    sort_idx = torch.sort(batch_ctx, stable=True).indices
    ...
    mask_ligand = torch.cat([
        torch.zeros([batch_protein.size(0)], device=batch_protein.device).bool(),
        torch.ones([batch_ligand.size(0)], device=batch_ligand.device).bool(),
        torch.zeros([refer_ligand_batch.size(0)], device=refer_ligand_batch.device).bool(),
    ], dim=0)[sort_idx]

    batch_ctx = batch_ctx[sort_idx]
    h_ctx = torch.cat([h_protein, h_ligand, refer_ligand_h], dim=0)[sort_idx]  # (N_protein+N_ligand, H)
    pos_ctx = torch.cat([pos_protein, pos_ligand, refer_ligand_pos], dim=0)[sort_idx]  # (N_protein+N_ligand, 3)
