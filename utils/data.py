...
def segment_and_mask_brics(mol=None):
    """
    Segment a molecule using BRICS rules and randomly mask one fragment.

    Args:
        mol (rdkit.Chem.Mol): Input molecule.

    Returns:
        np.ndarray: Boolean mask where True indicates atoms in the selected fragment.
    """
    if mol is None:
    ...
    # 1. Find BRICS-cleavable bonds
    # BRICS.FindBRICSBonds returns pairs of atom indices and bond type info
    brics_bonds_info = BRICS.FindBRICSBonds(mol)
    bonds_to_break = []
    for atom_idx_pair, _ in brics_bonds_info:
        atom_idx1, atom_idx2 = atom_idx_pair
        print(atom_idx1)
        print(atom_idx2)
        bond = mol.GetBondBetweenAtoms(atom_idx1, atom_idx2)
        if bond:  # Ensure the bond exists
            bonds_to_break.append(bond)

    # 2. Break bonds and retrieve fragments with original atom indices
    if not bonds_to_break:
        print("Warning: No BRICS bonds found. Treating the entire molecule as one fragment.")
        # If no cleavable bonds, consider the whole molecule as a single fragment
        all_fragments_atom_indices = (tuple(range(num_atoms)),)  # Single tuple with all atom indices
    else:
        # Fragment the molecule using Chem.FragmentOnBonds, addDummies=False avoids dummy atoms
        fragmented_mol = Chem.FragmentOnBonds(
            mol, [b.GetIdx() for b in bonds_to_break], addDummies=False
        )
        # Get atom indices for each fragment; asMols=False returns index tuples
        all_fragments_atom_indices = Chem.GetMolFrags(fragmented_mol, asMols=False)

    # 3. Randomly select one fragment
    if not all_fragments_atom_indices:
        # Theoretically unreachable unless molecule has no atoms
        selected_fragment_atoms_indices = tuple()
    else:
        selected_fragment_atoms_indices = random.choice(all_fragments_atom_indices)

    # 4. Build atom mask
    atom_mask = np.zeros(num_atoms, dtype=bool)  # Initially all atoms unmasked (False)
    for idx in selected_fragment_atoms_indices:
        atom_mask[idx] = True  # Mark atoms in the selected fragment as masked (True)
