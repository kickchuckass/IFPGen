...
def convert_ob_mol_to_rd_mol(ob_mol, struct=None):
    ...
    for ob_bond in ob.OBMolBondIter(ob_mol):
        ...
        # 检查键是否已存在
        existing_bond = rd_mol.GetBondBetweenAtoms(i, j)
        if existing_bond is not None:
            # 键已存在，更新键类型
            if bond_order == 1:
                existing_bond.SetBondType(Chem.BondType.SINGLE)
            elif bond_order == 2:
                existing_bond.SetBondType(Chem.BondType.DOUBLE)
            elif bond_order == 3:
                existing_bond.SetBondType(Chem.BondType.TRIPLE)
        else:
            # 键不存在，添加新键
            if bond_order == 1:
